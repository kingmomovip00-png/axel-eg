...
const state = { user:null, products:[], banners:[], coupons:[], images:[], editingId:null };
...
const el = {
  ...
  bannerFile:$('#bannerFile'), bannerPreview:$('#bannerPreview'), uploadBanner:$('#uploadBanner'), bannerProgress:$('#bannerProgress'), bannerMessage:$('#bannerMessage'), refreshBanners:$('#refreshBanners'), bannersList:$('#bannersList'), bannerProduct:$('#bannerProduct'), bannerOfferPrice:$('#bannerOfferPrice'), bannerOfferColor:$('#bannerOfferColor'), couponForm:$('#couponForm'), couponCode:$('#couponCode'), couponType:$('#couponType'), couponValue:$('#couponValue'), couponMin:$('#couponMin'), couponMax:$('#couponMax'), couponLimit:$('#couponLimit'), couponExpiry:$('#couponExpiry'), couponActive:$('#couponActive'), couponsList:$('#couponsList')
};
...
async function loadProducts(){
  ...
  try{const d=await api('/admin/products');state.products=d.products||[];if(el.bannerProduct) el.bannerProduct.innerHTML='<option value="">بدون منتج</option>'+state.products.map(p=>`<option value="${esc(p.id)}">${esc(p.name)} — ${money(p.salePrice)}</option>`).join('');
...
async function loadBanners(){el.bannersList.innerHTML='<p class="ax-help">جاري تحميل البانرات...</p>';try{const d=await api('/admin/banners');state.banners=d.banners||[];el.bannersList.innerHTML=state.banners.length?state.banners.map(b=>`<article class="ax-manager"><img src="${esc(b.imageUrl)}" alt="بانر"><div class="ax-manager-info"><b>بانر</b><span>${b.productId?`عرض: ${money(b.offerPrice)}`:'بدون عرض'} • ${b.active===false?'مخفي':'ظاهر'}</span></div><div class="ax-manager-actions"><button class="ax-btn ax-btn-light" type="button" data-toggle-banner="${esc(b.id)}" data-active="${b.active===true}">${b.active===false?'إظهار':'إخفاء'}</button><button class="ax-btn ax-btn-danger" type="button" data-delete-banner="${esc(b.id)}">حذف</button></div></article>`).join(''):'<p class="ax-help">لا توجد بانرات حتى الآن.</p>';}catch(e){el.bannersList.innerHTML=`<p class="ax-error">${esc(e.message)}</p>`;}}
async function uploadBanner(){const file=el.bannerFile.files?.[0];if(!file)return message('اختر صورة البانر أولاً.','error');if(!file.type.startsWith('image/'))return message('الملف المختار ليس صورة.','error');try{busy(el.uploadBanner,true,'جاري رفع البانر...');el.bannerProgress.style.width='25%';inline(el.bannerMessage,'جاري رفع الصورة...');const d=await upload([file],'/axel/banners');const image=d.files?.[0];if(!image?.url)throw new Error('تم الرفع لكن لم تصل بيانات الصورة من السيرفر.');el.bannerProgress.style.width='75%';await api('/admin/banners',{method:'POST',body:{imageUrl:image.url,fileId:image.fileId,productId:el.bannerProduct?.value||'',offerPrice:Number(el.bannerOfferPrice?.value||0),offerColor:el.bannerOfferColor?.value?.trim()||''}});el.bannerProgress.style.width='100%';inline(el.bannerMessage,'تم رفع البانر وإضافته للموقع بنجاح.','success');el.bannerFile.value='';el.bannerPreview.innerHTML='';await loadBanners();message('تمت إضافة البانر بنجاح.','success');}catch(e){el.bannerProgress.style.width='0';inline(el.bannerMessage,e.message,'error');message(`فشل رفع البانر: ${e.message}`,'error',true);}finally{busy(el.uploadBanner,false);}}

async function loadCoupons(){if(!el.couponsList)return;el.couponsList.innerHTML='<p class="ax-help">جاري تحميل الكوبونات...</p>';try{const d=await api('/admin/coupons');state.coupons=d.coupons||[];el.couponsList.innerHTML=state.coupons.length?state.coupons.map(c=>`<article class="ax-manager"><div class="ax-manager-info"><b>${esc(c.code)}</b><span>${c.type==='fixed'?money(c.value):`${Number(c.value||0)}%`} • استخدام ${Number(c.usedCount||0)}${Number(c.usageLimit||0)?`/${Number(c.usageLimit)}`:''}</span><small>${c.active===false?'موقوف':'فعال'}${c.minOrder?` • حد أدنى ${money(c.minOrder)}`:''}</small></div><div class="ax-manager-actions"><button class="ax-btn ax-btn-light" type="button" data-edit-coupon="${esc(c.code)}">تعديل</button><button class="ax-btn ax-btn-light" type="button" data-toggle-coupon="${esc(c.code)}" data-active="${c.active!==false}">${c.active===false?'تفعيل':'إيقاف'}</button><button class="ax-btn ax-btn-danger" type="button" data-delete-coupon="${esc(c.code)}">حذف</button></div></article>`).join(''):'<p class="ax-help">لا توجد كوبونات.</p>';}catch(e){el.couponsList.innerHTML=`<p class="ax-error">${esc(e.message)}</p>`;}}
async function saveCoupon(e){e.preventDefault();if(!el.couponForm)return;const body={code:el.couponCode.value.trim().toUpperCase(),type:el.couponType.value,value:Number(el.couponValue.value||0),minOrder:Number(el.couponMin.value||0),maxDiscount:Number(el.couponMax.value||0),usageLimit:Number(el.couponLimit.value||0),expiresAt:el.couponExpiry.value||null,active:el.couponActive.checked};try{await api('/admin/coupons',{method:'POST',body});message('تم إنشاء الكوبون بنجاح.','success');el.couponForm.reset();el.couponActive.checked=true;await loadCoupons();}catch(e){message(`فشل إنشاء الكوبون: ${e.message}`,'error',true);}}
...
el.couponForm?.addEventListener('submit',saveCoupon);el.couponsList?.addEventListener('click',async e=>{const ed=e.target.closest('[data-edit-coupon]'),t=e.target.closest('[data-toggle-coupon]'),d=e.target.closest('[data-delete-coupon]');try{if(ed){const c=state.coupons.find(x=>x.code===ed.dataset.editCoupon);if(c){const value=prompt('قيمة الخصم',c.value);if(value===null)return;const min=prompt('الحد الأدنى للطلب',c.minOrder||0);if(min===null)return;const max=prompt('أقصى خصم (0 بدون حد)',c.maxDiscount||0);if(max===null)return;const limit=prompt('حد الاستخدام (0 بدون حد)',c.usageLimit||0);if(limit===null)return;await api(`/admin/coupons/${encodeURIComponent(c.code)}`,{method:'PATCH',body:{value:Number(value),minOrder:Number(min),maxDiscount:Number(max),usageLimit:Number(limit)}})}}if(t)await api(`/admin/coupons/${encodeURIComponent(t.dataset.toggleCoupon)}`,{method:'PATCH',body:{active:t.dataset.active!=='true'}});if(d){if(!confirm('حذف الكوبون نهائيًا؟'))return;await api(`/admin/coupons/${encodeURIComponent(d.dataset.deleteCoupon)}`,{method:'DELETE'});}await loadCoupons();message('تم تحديث الكوبونات.','success')}catch(err){message(err.message,'error',true)}});
el.refreshAll.addEventListener('click',async()=>{busy(el.refreshAll,true,'جاري التحديث...');await Promise.all([loadStats(),loadProducts(),loadOrders(),loadBanners(),loadCoupons()]);busy(el.refreshAll,false);message('تم تحديث البيانات.','success');});el.logout.addEventListener('click',()=>signOut(auth));

onAuthStateChanged(auth,async user=>{if(!user||(user.email||'').toLowerCase()!==ADMIN_EMAIL.toLowerCase()){location.href='login.html';return;}state.user=user;el.adminEmail.textContent=user.email;renderOptions();renderImages();await Promise.all([loadStats(),loadProducts(),loadOrders(),loadBanners(),loadCoupons()]);});
